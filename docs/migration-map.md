...
### `helpers/`
* **Responsibility.** Procedural helpers for formatting, PDF generation, logging, tariff lookups, AI integration, search builders, etc. They are required directly from controllers and views to keep business logic concise. Example: `helpers/format.php` exposes `obtenerNombrePaciente(string $hcNumber, PDO $db)` and reaches into the database through a passed PDO instance.【F:helpers/format.php†L1-L10】
