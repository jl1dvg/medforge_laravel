...
$title = $titulo ?? 'Reporte de exámenes';
    ...
    <?php else: ?>
        <?php foreach ($rows as $index => $row): ?>
            ...
            <tr>
                ...
                <td><?= htmlspecialchars((string) ($row['examen_nombre'] ?? ($row['procedimiento'] ?? '—')), ENT_QUOTES, 'UTF-8') ?></td>
                <td><?= htmlspecialchars((string) $estadoLabel, ENT_QUOTES, 'UTF-8') ?></td>
                <td><?= htmlspecialchars((string) ($row['prioridad'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td>
                <td><?= htmlspecialchars($formatDate($row['consulta_fecha'] ?? ($row['created_at'] ?? null)), ENT_QUOTES, 'UTF-8') ?></td>
